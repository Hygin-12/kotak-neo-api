...
def handle_quote(quote):
    ...
    f = "HistoryIntra" + datetime.now().strftime("%d.%m.%Y") + ".csv"
    
    # Check if the file exists and is empty
    file_exists = os.path.isfile(f)
    is_empty = not os.path.getsize(f) > 0 if file_exists else False
    ...
    with open(f, mode='a', newline='') as f:
        writer = csv.writer(f)
...
# Logout and cleanup omitted for brevity
